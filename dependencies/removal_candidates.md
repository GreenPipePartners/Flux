...
| Fluxy optional `mcp` extra | Optional extra in `fluxy/pyproject.toml`; lazy import in `fluxy/mcp/server.py`; not observed in `uv pip list` despite tree/lock visibility. | Can pull ASGI/server transitive dependencies when installed. | **Keep optional only.** Do not install in web/runtime env by default. Remove later if MCP server is abandoned. | Medium |
| Fluxy optional `sqlalchemy` extra | Optional extra and lazy plugin import in `fluxy/plugins/sqlalchemy.py`; present in current fluxy env. | Adds ORM/database abstraction surface outside Django. | **Keep optional only.** Do not promote to runtime unless a concrete integration owns it. | Medium |
