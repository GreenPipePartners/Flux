...
from flux.chart.cache import PlaneSampleSyncResult, sync_plane_samples
    ...
    with transaction.atomic():
        for well_index, well in enumerate(wells, start=1):
            ...
            for tag_index, spec in enumerate(WELL_TRACE_TAGS, start=1):
                ...
                series = ensure_series_for_full_path(runtime_tag.full_path)
                tag_count += 1
...
def clear_nav_well_plane_samples(*, limit: int | None = None) -> int:
    keys = [well_profile_key(well.value) for well in navigation_wells(limit=limit)]
    series_ids = TraceSignal.objects.filter(profile__key__in=keys, series__isnull=False).values_list("series_id", flat=True)
    deleted_points, _details = Sample.objects.filter(series_id__in=series_ids).delete()
    TraceCacheCursor.objects.filter(signal__profile__key__in=keys).delete()
    return deleted_points


def sync_nav_well_plane_samples(fx: Any, *, limit: int | None = None, force: bool = False) -> PlaneSampleSyncResult:
    profiles = [well_profile_key(well.value) for well in navigation_wells(limit=limit)]
    profile_count = 0
    signal_count = 0
    point_count = 0
    for profile_key in profiles:
        result = sync_plane_samples(fx, profile_key=profile_key, force=force)
...
def seed_profile_cache(profile: TraceProfile, *, well_index: int, window_minutes: int = WELL_TRACE_WINDOW_MINUTES) -> int:
    ...
    start = end - timezone.timedelta(minutes=window_minutes)
    signals = list(profile.signals.select_related("tag", "series", "series__base_tag").order_by("sort_order"))
    points = []
