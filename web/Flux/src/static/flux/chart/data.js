...
export function mergeLiveSeries(liveTraceData, seriesPayload) {
  ...
  for (const incoming of seriesPayload.series) {
    const incomingKey = seriesIdentity(incoming);
    let existing = liveTraceData.find((series) => seriesIdentity(series) === incomingKey);
    if (!existing) {
      existing = { x: [], y: [], name: incoming.name, fullPath: incoming.fullPath, tagId: incoming.tagId, seriesId: incoming.seriesId, storageKey: incoming.storageKey, axisKey: incoming.axisKey, unit: incoming.unit };
...
function seriesIdentity(series) {
  return series.seriesId ?? series.storageKey ?? series.tagId;
}
...
function mergeSharedXSeries(liveTraceData, seriesPayload) {
  ...
  for (const incoming of seriesPayload.series) {
    ...
    let existing = liveTraceData.find((series) => seriesIdentity(series) === incomingKey);
    if (!existing) {
      existing = { x: [], y: new Array(liveTraceData.sharedX.length).fill(null), name: incoming.name, fullPath: incoming.fullPath, tagId: incoming.tagId, seriesId: incoming.seriesId, storageKey: incoming.storageKey, axisKey: incoming.axisKey, unit: incoming.unit };
      liveTraceData.push(existing);
    }
